# =================== LSM HYBRID (CNN-LGBM) with Progress + Figures + Metrics ===================
# CNN (GPU) for deep features + LightGBM for classifier.
# Version-safe LightGBM fit, CSV padding, numerically safe stats,
# compressed/tiling/BigTIFF outputs, disk-space guard, consistent feature names,
# and checkpoint shape ADAPTATION (handles proj weight/bias mismatches automatically).

import os, warnings, math, numpy as np, joblib, time, random, shutil
...
DEEP_FEATS_TIF = '/home/capybara/Works/2025_landslide/CNN_LGBM/process/deep_feats.tif'
MODEL_PATH     = '/home/capybara/Works/2025_landslide/CNN_LGBM/model/lgbm_fused_points.joblib'
OUT_PROB_TIF   = '/home/capybara/Works/2025_landslide/CNN_LGBM/result/lsm_prob.tif'
...
# ---------- CSV helpers (pad thresholds to align lengths) ----------------------------
...
# --- GeoTIFF IO knobs ---------------------------------------------------------------
TIFF_COMPRESS   = "LZW"    # "ZSTD" if available
TIFF_BLOCK      = 256
TIFF_BIGTIFF    = "YES"
AUTO_SHRINK_DEEP_CHANNELS_TO_FIT_DISK = True
DISK_SAFETY_FRACTION = 0.85  # use up to 85% of free space

# --- LightGBM ------------------------------------------------------------------------
LGBM_N_JOBS = max(1, (os.cpu_count() or 1))
LGBM_USE_GPU = False         # set True only if your LightGBM build has GPU
...
def compute_band_stats(src, block=1024):
    """Numerically safe streaming stats using float64 accumulators."""
    ...
    for y in range(0, src.height, block):
        for x in range(0, src.width, block):
            ...
            s = mask.sum(axis=(1,2), dtype="int64")
            ...
            if means is None:
                means, sqs, counts = m, q, s
            else:
                means += m; sqs += q; counts += s
    counts = np.maximum(counts, 1)
    means = means / counts
    var = np.maximum(sqs / counts - means*means, 1e-12)
    std = np.sqrt(var)
    return means.astype("float32"), std.astype("float32")

def _standardize_block(arr, mean, std, clip_std_min=1e-3):
    arr = np.nan_to_num(arr, copy=False)
    std_safe = np.maximum(std, clip_std_min)
...
def _adapt_and_load_encoder_ckpt(model: nn.Module, ckpt_path: str):
    """
    Loads encoder checkpoint and adapts 'proj' (1x1 conv) weight/bias if out_ch differs.
    - If src_out > tgt_out  -> slice first tgt_out channels.
    - If src_out < tgt_out  -> copy and initialize the remaining channels.
    Falls back to non-strict loading for other minor diffs.
    """
    try:
        state = torch.load(ckpt_path, map_location="cpu")
    except Exception as e:
        warnings.warn(f"[A] Could not read encoder ckpt ({e}); continuing without it.")
        return
    ...
    # Try to adapt projection layer if present
    try:
        if "proj.weight" in state and "proj.bias" in state and "proj.weight" in msd:
            w_src = state["proj.weight"]
            b_src = state["proj.bias"]
            out_s, in_s, _, _ = w_src.shape
            out_t, in_t, _, _ = msd["proj.weight"].shape
            # Ensure in channels match (should for same backbone); if not, drop proj
            if in_s != in_t:
                warnings.warn("[A] proj in-ch mismatch; skipping proj from ckpt.")
                state.pop("proj.weight", None); state.pop("proj.bias", None)
            else:
                dtype = msd["proj.weight"].dtype
                device = msd["proj.weight"].device
                if out_s == out_t:
                    # nothing to do
                    pass
                elif out_s > out_t:
                    ...
                    print(f"[A][ckpt] proj channels {out_s}→{out_t} (slice).")
                else:
                    # pad remaining channels with Kaiming init
                    w_new = torch.empty((out_t, in_t, 1, 1), dtype=dtype, device=device)
                    b_new = torch.empty((out_t,), dtype=dtype, device=device)
                    w_new[:out_s] = w_src.to(dtype)
                    b_new[:out_s] = b_src.to(dtype)
                    nn.init.kaiming_normal_(w_new[out_s:], a=0.0, mode="fan_in", nonlinearity="relu")
                    nn.init.zeros_(b_new[out_s:])
                    state["proj.weight"] = w_new.cpu()
                    state["proj.bias"]   = b_new.cpu()
                    print(f"[A][ckpt] proj channels {out_s}→{out_t} (pad/init).")
    except Exception as e:
        warnings.warn(f"[A] proj adaptation failed ({e}); loading non-strictly.")

    missing, unexpected = model.load_state_dict(state, strict=False)
    if missing or unexpected:
        print(f"[A][ckpt] loaded with missing={list(missing)}, unexpected={list(unexpected)}")
...
def write_deep_feature_raster(in_tif, out_tif=DEEP_FEATS_TIF,
    ...
    # Plan disk usage and auto-shrink K
    ...
    if AUTO_SHRINK_DEEP_CHANNELS_TO_FIT_DISK and req_bytes > budget:
        ...
        if max_k < feat_channels:
            print(f"[A][disk] Need ~{_bytes_human(req_bytes)} but only {_bytes_human(free_bytes)} free.")
            print(f"[A][disk] Shrinking deep channels: {feat_channels} → {max_k} "
                  f"(safety {int(DISK_SAFETY_FRACTION*100)}%).")
            feat_channels = int(max_k)

    meta = src_meta.copy()
    meta.update(
        count=feat_channels, dtype="float32",
        tiled=True, blockxsize=TIFF_BLOCK, blockysize=TIFF_BLOCK,
        compress=TIFF_COMPRESS, bigtiff=TIFF_BIGTIFF,
    )

    model = FCNFeatureExtractor(in_ch=C, out_ch=feat_channels, backbone=BACKBONE).eval()

    # <<< NEW: adaptively load encoder ckpt (handles channel mismatch) >>>
    ...
    est_bytes = int(width) * int(height) * int(feat_channels) * bytes_per_pixel
    print(f"[A] Wrote deep features → {out_tif}  (≈{_bytes_human(est_bytes)})")
    _log_time("A: deep_features", dt, tiles=n_tiles, tile=tile, halo=HALO,
              K=feat_channels, backbone=BACKBONE,
              device=("CUDA" if torch.cuda.is_available() and USE_GPU_IF_AVAILABLE else "CPU"),
              batch=GPU_TILE_BATCH, compress=TIFF_COMPRESS, bigtiff=TIFF_BIGTIFF)
...
# ================= LightGBM helpers (version-safe fit) ==============================
def _lgbm_device_params():
    if LGBM_USE_GPU:
        return dict(device_type='gpu')
    else:
...
# ---------- Random search over compact space (LightGBM) ----------
def tune_lgbm_params(
    ...
    def _sub(Xin, idx):  # DataFrame vs ndarray safe
        return Xin.iloc[idx] if isinstance(Xin, pd.DataFrame) else Xin[idx]
    ...
    for i in range(n_iter):
        ...
        params.update(cand)
        params.update(
            n_estimators=4000,                 # let early stopping pick best_iter
...
def _plot_lgbm_tuning_results(history, metric="AP", out_dir=FIG_DIR, csv_dir=LOG_DIR):
    ...
    if {"learning_rate","num_leaves"}.issubset(df.columns):
        ...
        plt.xlabel("learning_rate"); plt.ylabel("num_leaves")
        plt.title("LGBM tuning: score by (learning_rate, num_leaves)")
...
def train_lgbm_from_points(deep_feats_tif=DEEP_FEATS_TIF, factors_tif=FACTOR_TIF,
    ...
    # Features (stable column names; keep DataFrame to silence warnings)
    ...
    raw_cols  = [f"raw_{i}"  for i in range(Xraw.shape[1])]
    ...
    tr_idx, va_idx = next(gss.split(Xdf, y, groups))
    Xtr, ytr, Xva, yva = Xdf.iloc[tr_idx], y[tr_idx], Xdf.iloc[va_idx], y[va_idx]
    ...
    base_core  = dict(objective="binary", metric="binary_logloss", random_state=SEED, verbosity=-1)
    t0_tune = time.perf_counter()
    best_params, best_cv, tune_hist = tune_lgbm_params(
        Xdf, y, groups,
        base_params={**base_core, **base_flags},
        metric="ap",
        n_iter=LGBM_TUNE_ITER,
        n_splits=min(5, n_splits),
        seed=SEED,
        early_stopping_rounds=EARLY_STOPPING_ROUNDS,
        pos_weight=spw,
    )
    _plot_lgbm_tuning_results(tune_hist, metric="AP", out_dir=FIG_DIR, csv_dir=LOG_DIR)
    _log_time("B: lgbm_tune", time.perf_counter()-t0_tune, iters=LGBM_TUNE_ITER)

    # ====== Final model: refit on train with ES on valid ======
    if best_params is None:
        best_params = {**base_core, **base_flags}
    print("[B] Best LGBM params (subset):", {k: best_params.get(k) for k in
    ...
    best_iter = getattr(base, "best_iteration_", None)
    _log_time("B: lgbm_fit", time.perf_counter()-t0_fit,
              early_stopping=("yes" if best_iter is not None else "no"))

    # --- Logloss curve
    try:
        evals = base.evals_result_
        if isinstance(evals, dict):
            plt.figure()
            if "training" in evals and "binary_logloss" in evals["training"]:
                plt.plot(evals["training"]["binary_logloss"], label="train")
            if "valid_1" in evals and "binary_logloss" in evals["valid_1"]:
                plt.plot(evals["valid_1"]["binary_logloss"], label="valid")
            plt.xlabel("Boosting round"); plt.ylabel("Logloss"); plt.legend()
            plt.title("LGBM logloss (train vs valid)")
            _savefig(os.path.join(FIG_DIR, "lgbm_logloss.png"))
    except Exception:
        pass
    ...
    _curve_df_with_thresholds(fpr_arr, tpr_arr, thr_roc, "fpr", "tpr").to_csv(
        os.path.join(LOG_DIR, "roc_valid.csv"), index=False)
    prec_arr, rec_arr, thr_pr = precision_recall_curve(yva, p_va)
    _curve_df_with_thresholds(rec_arr, prec_arr, thr_pr, "recall", "precision").to_csv(
        os.path.join(LOG_DIR, "pr_valid.csv"), index=False)
    ...
    with tqdm(total=n_splits, desc="[B] Spatial CV") as pbar:
        for tr, te in gkf.split(Xdf, y, groups):
            base_cv = LGBMClassifier(**best_params)
            base_cv.fit(Xdf.iloc[tr], y[tr])  # no ES in CV loop
            p = base_cv.predict_proba(Xdf.iloc[te])[:,1]
    ...
    joblib.dump({"clf": clf, "best_iter": best_iter,
                 "valid_metrics": summary, "thresholds": {"best_t": best_t},
                 "feature_names": feature_names}, out_model)
            ...
            with rasterio.open(out_tif, 'w', **meta) as dst, tqdm(total=total, desc="[C] Map predict") as pbar:
                for y in range(0, height, tile):
                    for x in range(0, width, tile):
                        ...
                        # Keep identical feature names as training (kills the sklearn warning)
                        if feature_names is not None and len(feature_names) == X.shape[1]:
                            X_pred = pd.DataFrame(X, columns=feature_names)
                        else:
                            X_pred = X

                        P = clf.predict_proba(X_pred)[:,1].astype('float32').reshape(H, W)
...
if __name__ == "__main__":
    ...
    train_lgbm_from_points(DEEP_FEATS_TIF, FACTOR_TIF, POINTS_PATH, out_model=MODEL_PATH, n_splits=N_SPLITS)
