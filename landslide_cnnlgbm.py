# =================== LSM HYBRID (CNN-LGBM) v3 — Spatial CV + Discussion Diagnostics ===================
# You have to change the path according to your system.
# ===============================================================================================

import os, warnings, math, numpy as np, joblib, time, random, shutil, gc
...
DEEP_FEATS_TIF = f'{BASE}/CNN_LGBM/process/deep_feats.tif'
MODEL_PATH     = f'{BASE}/CNN_LGBM/model/lgbm_fused_points.joblib'
OUT_PROB_TIF   = f'{BASE}/CNN_LGBM/result/lsm_prob.tif'
...
# --- NEW in v3: Raw band names for conditioning factors ------------------------------
# Update to match the band order in your Raster_Com.tif (same as CNN-XGB v5).
RAW_BAND_NAMES = [
...
#   GPU_TILE_BATCH=8 → 8×134 MB = 1.07 GB + model 120 MB ≈ 1.5 GB / 11 GB
#   LightGBM training is CPU-only (LGBM_USE_GPU=False for this script) but
#   CNN inference still uses the GPU → batch size 8 is safe.
# Fine-tuning: gradients only for unfrozen layers (layer3, layer4, proj)
#   FT_BATCH_TILES=6 → ~2.5 GB — comfortable margin below 11 GB
# RAM: 128 GB → Moran's I samples 10,000 points (vs 3,000 default)
#              → LightGBM can hold full dataset in RAM without paging
#              → LGBM_N_JOBS = all cores for maximum throughput
# Note: LGBM_USE_GPU=False because LightGBM GPU only supports
#   limited objective types and is less stable than CPU for this use case.
#   All 11 GB VRAM is reserved for CNN inference + fine-tuning.
# =============================================================================
...
# Peak per batch (FP16 inference, in-place TTA):
#   model ~120 MB + intermediates ~130 MB + outs(FP32) batch×268 MB + fk(FP16) batch×134 MB
#   At batch=4: 120+130+4×402 = ~1.9 GB ← very safe, leaves 9 GB for LightGBM + OS
GPU_TILE_BATCH       = 4      # was 8 → reduced to safe value; in-place TTA eliminates OOM
torch.set_float32_matmul_precision("high")  # TF32 on Ampere; high-perf matmul on all

TIFF_COMPRESS   = "LZW"
TIFF_BLOCK      = 256
TIFF_BIGTIFF    = "YES"
AUTO_SHRINK_DEEP_CHANNELS_TO_FIT_DISK = True
DISK_SAFETY_FRACTION = 0.85

LGBM_N_JOBS           = max(1, (os.cpu_count() or 1))  # all CPU cores for LGBM
LGBM_USE_GPU          = False   # CPU mode — full 11 GB VRAM stays for CNN
CALIBRATION_METHOD    = None
EARLY_STOPPING_ROUNDS = 500     # ↓ was 1000 → sufficient patience; saves ~2× time per fold
LGBM_TUNE_ITER        = 150     # ↑ was 100 → wider search; parallel tuning absorbs cost
...
FT_BATCH_TILES       = 4      # was 6 → reduced; fine-tuning carries model+optimizer (~4.5 GB)
FT_ACCUM_STEPS       = 1      # stays 1 — batch is sufficient; no accumulation needed
    ...
    with tqdm(total=n_bx*n_by, desc="[A] Band statistics", unit="block", leave=False) as pbar:
        for y in range(0, src.height, block):
            for x in range(0, src.width, block):
                ...
                arr = src.read(window=w, out_dtype="float32")
                mask = np.isfinite(arr); s = mask.sum(axis=(1,2), dtype="int64")
                arr = np.where(mask, arr, 0.0).astype("float64", copy=False)
                m = arr.sum(axis=(1,2), dtype="float64")
                q = np.square(arr, dtype="float64").sum(axis=(1,2), dtype="float64")
                if means is None: means, sqs, counts = m, q, s
                else: means += m; sqs += q; counts += s
                pbar.update(1)
    counts = np.maximum(counts, 1)
    means  = means / counts
    var    = np.maximum(sqs / counts - means*means, 1e-12)
    return means.astype("float32"), np.sqrt(var).astype("float32")

def _standardize_block(arr, mean, std, clip_std_min=1e-3):
    arr = np.nan_to_num(arr, copy=False)
    return (arr - mean[:,None,None]) / np.maximum(std, clip_std_min)[:,None,None]
...
# =============================================================================
# FIX 1 (Issue 2): _make_spatial_blocks — larger blocks for better separation
# v2: nx = ny = ceil(sqrt(n_splits*2))  → n_splits=5: 4×4=16 cells (too small)
# v3: n_cells = ceil(sqrt(n_splits))    → n_splits=5: 3×3= 9 cells (larger)
# =============================================================================
...
# --- Checkpoint adaptation (unchanged from v2) ----------------------------------------
def _adapt_and_load_encoder_ckpt(model, ckpt_path):
    try:
        state = torch.load(ckpt_path, map_location="cpu")
    except Exception as e:
        warnings.warn(f"[A] Could not read encoder ckpt ({e}); continuing without it."); return
    if isinstance(state, dict) and "state_dict" in state:
        state = state["state_dict"]
    msd = model.state_dict()
    try:
        if "proj.weight" in state and "proj.bias" in state and "proj.weight" in msd:
            w_src = state["proj.weight"]; b_src = state["proj.bias"]
            out_s,in_s,_,_ = w_src.shape; out_t,in_t,_,_ = msd["proj.weight"].shape
            if in_s != in_t:
                warnings.warn("[A] proj in-ch mismatch; skipping proj from ckpt.")
                state.pop("proj.weight",None); state.pop("proj.bias",None)
            else:
                dtype = msd["proj.weight"].dtype; device = msd["proj.weight"].device
                if out_s > out_t:
                    state["proj.weight"] = w_src[:out_t,:,:,:].to(dtype=dtype)
                    state["proj.bias"]   = b_src[:out_t].to(dtype=dtype)
                    print(f"[A][ckpt] proj {out_s}→{out_t} (slice).")
                elif out_s < out_t:
                    w_new = torch.empty((out_t,in_t,1,1),dtype=dtype,device=device)
                    b_new = torch.empty((out_t,),dtype=dtype,device=device)
                    w_new[:out_s]=w_src.to(dtype); b_new[:out_s]=b_src.to(dtype)
                    nn.init.kaiming_normal_(w_new[out_s:],a=0.0,mode="fan_in",nonlinearity="relu")
                    nn.init.zeros_(b_new[out_s:])
                    state["proj.weight"]=w_new.cpu(); state["proj.bias"]=b_new.cpu()
                    print(f"[A][ckpt] proj {out_s}→{out_t} (pad/init).")
    except Exception as e:
        warnings.warn(f"[A] proj adaptation failed ({e}); loading non-strictly.")
    missing, unexpected = model.load_state_dict(state, strict=False)
    if missing or unexpected:
        print(f"[A][ckpt] missing={list(missing)}, unexpected={list(unexpected)}")

# --- LightGBM helpers (unchanged from v2) --------------------------------------------
def _lgbm_device_params():
    # n_jobs set in ALL paths — LightGBM uses CPU threads even in GPU mode
    # (data preprocessing, metric evaluation). Default is -1 (all cores) but explicit is safer.
    if LGBM_USE_GPU:
        return dict(device_type='gpu', n_jobs=LGBM_N_JOBS)
    return dict(device_type='cpu', n_jobs=LGBM_N_JOBS)

def _safe_lgbm_fit(model, X, y, *, eval_set=None, metric_name="logloss",
                   early_stopping_rounds=None, verbose_eval=False):
    """Try multiple .fit() signatures; fallback to callbacks when kwargs unsupported."""
    attempts = [
        dict(eval_set=eval_set, eval_metric=metric_name,
             early_stopping_rounds=early_stopping_rounds, verbose=verbose_eval),
        dict(eval_set=eval_set, eval_metric=metric_name,
             callbacks=(([lgb.early_stopping(early_stopping_rounds, verbose=verbose_eval)]
                        if early_stopping_rounds else []) +
                        ([] if verbose_eval else [lgb.log_evaluation(period=0)]))),
        dict(eval_set=eval_set,
             callbacks=(([lgb.early_stopping(early_stopping_rounds, verbose=verbose_eval)]
                        if early_stopping_rounds else []) +
                        ([] if verbose_eval else [lgb.log_evaluation(period=0)]))),
        dict()
...
# --- Tuning — parallelised across iterations (CPU mode only; GPU stays sequential) ---
def tune_lgbm_params(X, y, groups, *, _tune_gdf=None, base_params, metric="ap", n_iter=20,
                     n_splits=5, seed=42, early_stopping_rounds=400, pos_weight=1.0):
    ...
    def _sub(Xin, idx): return Xin.iloc[idx] if isinstance(Xin, pd.DataFrame) else Xin[idx]

    # Pre-draw candidates for reproducibility before any parallelism
    ...
    def _eval_candidate(i, cand):
        ...
        for tr, va in _tune_splits:
            ...
            _safe_lgbm_fit(m, _sub(X,tr), y[tr],
                           eval_set=[(_sub(X,va), y[va])],
                           metric_name="logloss",
                           early_stopping_rounds=early_stopping_rounds,
                           verbose_eval=False)
            fold_scores.append(mfn(y[va], m.predict_proba(_sub(X,va))[:,1]))
        ms = float(np.mean(fold_scores))
        print(f"[B][tune-LGBM] {i+1}/{n_iter} AP={ms:.4f}")
        return ms, cand, dict(params)

    # GPU: keep sequential (single device, can't run parallel trainers).
    # CPU: parallelise across iterations using threads.
    use_gpu = base_params.get("device_type", "cpu") == "gpu"
    ...
    print(f"[B][tune-LGBM] BEST AP={best_score:.4f}")
    return best_params, best_score, history

def _plot_lgbm_tuning_results(history, metric="AP", out_dir=FIG_DIR, csv_dir=LOG_DIR):
    if not history: return
    df = pd.DataFrame(history)
    df.to_csv(os.path.join(csv_dir, "lgbm_tuning_history.csv"), index=False)
    plt.figure(figsize=(6,4)); plt.plot(df["iter"], df["score"], marker="o")
    plt.xlabel("Iteration"); plt.ylabel(metric); plt.title(f"LGBM tuning: {metric}")
    _savefig(os.path.join(out_dir, "lgbm_tuning_curve.png"))
    if {"learning_rate","num_leaves"}.issubset(df.columns):
        plt.figure(figsize=(6,5))
        sc = plt.scatter(df["learning_rate"], df["num_leaves"], c=df["score"])
        plt.colorbar(sc, label=metric); plt.xlabel("learning_rate"); plt.ylabel("num_leaves")
        plt.title("LGBM tuning: score by (lr, num_leaves)")
        _savefig(os.path.join(out_dir, "lgbm_tuning_scatter_lr_leaves.png"))


# =============================================================================
# NEW IN v3 — DIAGNOSTIC FUNCTIONS FOR DISCUSSION SECTION (ISSUE 3)
# Identical logic to CNN-XGB v5; D4 adapted for LightGBM booster API.
# =============================================================================

def _compute_morans_i(residuals, coords, k=8, n_sample=MORANS_N_SAMPLE, seed=42):
    """
    [D2] Moran's I on OOF residuals — quantifies spatial autocorrelation of errors.
    Uses k-NN binary weights + 499-permutation p-value.
    """
    ...
    nn_idx = nn_idx[:, 1:]   # drop self-index (first column is always the point itself)
    z         = r - r.mean()
    ...
    if I>0.3 and p_val<0.05:
        interp = (f"Strong positive spatial autocorrelation (I={I:.3f}, p={p_val:.3f}). "
                  "Model errors cluster spatially. Spatial CV was necessary.")
...
def _plot_overfitting_analysis(fold_df, out_dir=FIG_DIR, log_dir=LOG_DIR):
    ...
    axes[0].bar(x+w/2, test_auc,  w, label="Test AUROC",  color="#6ACC65")
    ...
    axes[0].set_ylabel("AUROC"); axes[0].set_title("Train vs Test AUROC per Fold"); axes[0].legend()
    ...
    axes[1].axhline(0.05, linestyle="--", color="red", lw=1.5, label="Threshold (0.05)")
...
def _plot_calibration_curve_diag(y_true, p_pred, n_bins=10, out_dir=FIG_DIR, log_dir=LOG_DIR):
    ...
    axes[0].plot(mean_pred, frac_pos, 's-', color='#4878CF', lw=2,
                 label=f'CNN-LightGBM (ECE={ece:.3f})')
...
def _plot_feature_importance_lgbm(clf_final, feature_names, out_dir=FIG_DIR, log_dir=LOG_DIR, top_n=30):
    """
    [D4] LightGBM gain-based feature importance.
    Uses booster_.feature_importance(importance_type='gain') — LGBM-specific API.
    """
    try:
        booster   = clf_final.booster_
        gain_vals = booster.feature_importance(importance_type='gain')
        names     = booster.feature_name()
    except Exception:
        try:
            # Fallback: sklearn wrapper gives split importance but at least something
            gain_vals = clf_final.feature_importances_
            names     = feature_names[:len(gain_vals)] if feature_names else [f"f{i}" for i in range(len(gain_vals))]
            warnings.warn("[D4] Using split importance (booster_ unavailable). Gain preferred.")
        except Exception:
            print("[D4] Could not retrieve feature importance — skipping."); return None

    imp_df = pd.DataFrame({"feature": list(names), "gain": gain_vals.astype(float)})
    ...
    colors  = ["#4878CF" if "deep_" in str(r["feature"]) or "CNN" in str(r["feature"])
               else "#D65F5F" for _, r in plot_df.iterrows()]

    fig, ax = plt.subplots(figsize=(10, max(5, len(plot_df)*0.35)))
    ax.barh(plot_df["feature"][::-1], plot_df["gain_pct"][::-1], color=colors[::-1])
    ax.set_xlabel("Gain Importance (%)"); ax.set_title(f"LightGBM Feature Importance (top {top_n}, gain)")
...
def _generate_discussion_report(cv_summary, fold_df, mean_gap, overfitting_flag,
    ...
    report = f"""
=======================================================================
DISCUSSION SECTION DRAFT — CNN-LightGBM — AUTO-GENERATED FROM DIAGNOSTICS
=======================================================================
[Copy and refine into your manuscript. Replace [REF] with citations.]
=======================================================================

--- Performance Interpretation ---
The CNN-LightGBM hybrid model was evaluated using spatial k-fold
cross-validation (n={N_SPLITS} folds) with geographically separated blocks,
preventing spatial leakage between training and test samples [REF].
...
deviation across folds reflects geographic variability in model
...
generalises to geographically unseen areas. Regularisation through
LightGBM hyperparameters (L1/L2 penalties, leaf-wise growth strategy,
min_child_samples, subsampling) and spatial cross-validation contributed
to this outcome [REF]. Nevertheless, reported metrics represent
comparative model skill under the adopted spatial validation framework
rather than fully independent operational performance estimates.

--- Spatial Autocorrelation of Residuals (D2) ---
Moran's I on the out-of-fold prediction residuals was {morans_I:.4f}
(p = {morans_p:.3f}). {morans_interp}
This demonstrates the appropriateness of spatial cross-validation for
this dataset, as a random split would allow spatially autocorrelated
samples to leak between folds, inflating apparent performance [REF].

--- Calibration and Practical Applicability (D3) ---
The Expected Calibration Error (ECE) from the reliability diagram was
{calibration_error:.4f}. {'An ECE below 0.05 indicates well-calibrated probability outputs.' if calibration_error<0.05 else 'Moderate miscalibration suggests treating outputs as relative rankings rather than absolute probabilities.'}
The framework is a susceptibility assessment tool, not a real-time
early warning system. It excludes dynamic triggering variables (rainfall
intensity, antecedent moisture). High or very high susceptibility zones
should be verified through field inspection and expert knowledge.

--- Feature Importance and Physical Interpretability (D4) ---
The five most important factors by LightGBM gain importance were:
{top5 if top5 else '[see feature_importance.csv]'}. This ranking aligns
with physical controls on landslide occurrence: topographic factors
govern gravitational stress, while geological factors control material
strength. CNN deep features in the top-5 confirm that spatial feature
extraction contributed predictive information beyond raw conditioning
factors, consistent with hybrid LSM literature [REF].

--- Limitations ---
The landslide inventory consists of historical point records with
uncertain positional accuracy for pre-2000 events. The balanced
sampling strategy does not reflect the naturally imbalanced landscape
distribution. The model is calibrated for Uttaradit Province and
requires recalibration before application to other regions. Future
work should integrate dynamic triggering factors, apply temporally
separated validation, and test transferability across regions.
...
def _run_discussion_diagnostics(fold_df, all_preds_df, clf_final, feature_names,
    ...
    """Master diagnostic runner — D1 through D5."""
    ...
    with tqdm(total=len(steps), desc="[D] Diagnostics", unit="step") as dpbar:
        ...
        imp_df = _plot_feature_importance_lgbm(clf_final, feature_names, out_dir, log_dir, top_n=30)
        dpbar.update(1)
...
# =============================================================================
# FIX 2 (Issue 2 + 3): train_lgbm_from_points — spatial k-fold CV + diagnostics
# =============================================================================
def train_lgbm_from_points(deep_feats_tif=DEEP_FEATS_TIF, factors_tif=FACTOR_TIF,
                           points_path=POINTS_PATH, out_model=MODEL_PATH, n_splits=N_SPLITS):
    t0_total = time.perf_counter()
    print("[B] Training CNN-LightGBM with spatial k-fold CV + discussion diagnostics …")
    ...
    # ------------------------------------------------------------------
    # Features — DataFrame for named columns (LGBM-specific, unchanged)
    # ------------------------------------------------------------------
    ...
    deep_cols = [f"deep_{i}" for i in range(Xdeep.shape[1])]
    # Use meaningful names for raw bands (from RAW_BAND_NAMES constant)
    n_raw = Xraw.shape[1]
    if len(RAW_BAND_NAMES) == n_raw:
        raw_cols = list(RAW_BAND_NAMES)
    else:
        warnings.warn(f"RAW_BAND_NAMES has {len(RAW_BAND_NAMES)} entries but raster has "
                      f"{n_raw} bands. Using generic names. Update RAW_BAND_NAMES.")
        raw_cols = [f"raw_{i}" for i in range(n_raw)]

    feature_names = deep_cols + raw_cols
    X    = np.hstack([Xdeep, Xraw]).astype('float32')
    X    = np.nan_to_num(X, nan=0.0, posinf=0.0, neginf=0.0)
    Xdf  = pd.DataFrame(X, columns=feature_names)   # DataFrame for LGBM feature names
    _log_time("B: sample_features", time.perf_counter()-t0s,
              N=len(y), Kdeep=Xdeep.shape[1], Kraw=n_raw)

    # ------------------------------------------------------------------
    # Spatial blocks (larger, better separation)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Step A — Hyperparameter tuning (spatial GroupKFold, unchanged)
    # ------------------------------------------------------------------
    base_flags = _lgbm_device_params()
    base_core  = dict(objective="binary", metric="binary_logloss",
                      random_state=SEED, verbosity=-1)
    t0t = time.perf_counter()
    best_params, best_cv, tune_hist = tune_lgbm_params(
        Xdf, y, groups, _tune_gdf=gdf,
        base_params={**base_core, **base_flags},
        metric="ap", n_iter=LGBM_TUNE_ITER,
        n_splits=min(5, n_splits), seed=SEED,
        early_stopping_rounds=EARLY_STOPPING_ROUNDS, pos_weight=spw)
    _plot_lgbm_tuning_results(tune_hist); _log_time("B: lgbm_tune", time.perf_counter()-t0t)
    if best_params is None:
        best_params = {**base_core, **base_flags}
    ...
    with tqdm(total=n_splits, desc="[B] Spatial CV folds") as pbar:
        for fold_i, (tr_idx, te_idx) in enumerate(cv_splits):
            Xtr_df = Xdf.iloc[tr_idx]; ytr = y[tr_idx]
            Xte_df = Xdf.iloc[te_idx]; yte = y[te_idx]
            ...
            clf_fold = LGBMClassifier(**fold_params)
            _safe_lgbm_fit(clf_fold, Xtr_df, ytr,
                           eval_set=[(Xte_df, yte)],
                           metric_name="logloss",
                           early_stopping_rounds=EARLY_STOPPING_ROUNDS,
                           verbose_eval=False)

            # LGBM uses best_iteration_ (trailing underscore)
            bi = getattr(clf_fold, "best_iteration_", None)
            if bi is not None and bi > 0: best_iters.append(int(bi))

            p_te = clf_fold.predict_proba(Xte_df)[:,1]

            # ----------------------------------------------------------------
            # NEW IN v3 (D1): train AUROC per fold for overfitting analysis
            # ----------------------------------------------------------------
            p_tr        = clf_fold.predict_proba(Xtr_df)[:,1]
    ...
    # n_estimators = mean best_iteration from CV folds
    # Used ONLY for the probability map — NOT for metric reporting
    # ------------------------------------------------------------------
    mean_best_iter = int(round(np.mean(best_iters))) if best_iters else 500
    print(f"\n[B] Final model: ALL data | n_estimators={mean_best_iter} "
          f"(mean of per-fold best_iter: {best_iters})")
    ...
    clf_final = LGBMClassifier(**final_params)
    # No early stopping for final model — no held-out set, fixed n_estimators
    _safe_lgbm_fit(clf_final, Xdf, y,
                   eval_set=None,
                   early_stopping_rounds=None,
                   metric_name="binary_logloss",
                   verbose_eval=False)
    _log_time("B: lgbm_final_fit", time.perf_counter()-t0f, N=len(y),
              n_estimators=mean_best_iter)
    print("[B] Final model trained on all data → used for probability map only")
...
# ================= Deep features raster (unchanged from v2) ==========================
@torch.no_grad()
def write_deep_feature_raster(in_tif, out_tif=DEEP_FEATS_TIF,
    ...
    with rasterio.open(in_tif) as src:
        ...
        width,height=src.width,src.height; src_meta=src.meta.copy()
    bytes_per_pixel = np.dtype("float32").itemsize
    free_bytes = shutil.disk_usage(os.path.dirname(out_tif) or ".").free
    req_bytes  = int(width)*int(height)*int(feat_channels)*bytes_per_pixel
    budget     = int(free_bytes*DISK_SAFETY_FRACTION)
    if AUTO_SHRINK_DEEP_CHANNELS_TO_FIT_DISK and req_bytes>budget:
        max_k = max(1, budget//(int(width)*int(height)*bytes_per_pixel))
        if max_k<feat_channels:
            print(f"[A][disk] Shrinking channels {feat_channels}→{max_k}")
            feat_channels = int(max_k)
    meta = src_meta.copy()
    meta.update(count=feat_channels,dtype="float32",tiled=True,
                blockxsize=TIFF_BLOCK,blockysize=TIFF_BLOCK,
                compress=TIFF_COMPRESS,bigtiff=TIFF_BIGTIFF)
    ...
    if encoder_ckpt and os.path.exists(encoder_ckpt):
        _adapt_and_load_encoder_ckpt(model, encoder_ckpt)
    if device.type=="cuda":
    ...
    with rasterio.Env(CHECK_DISK_FREE_SPACE=False):
        with rasterio.open(out_tif,"w",**meta) as dst,rasterio.open(in_tif) as src,\
             tqdm(total=n_tiles,desc=desc) as pbar:
            if device.type=="cuda":
                ...
                for(x,y,w,h) in tiles:
                    ...
                    pbar.update(1)
    _cuda_sync_if_any()
    est=int(width)*int(height)*int(feat_channels)*bytes_per_pixel
    print(f"[A] → {out_tif}  (≈{_bytes_human(est)})")
    _log_time("A: deep_features",time.perf_counter()-t0,tiles=n_tiles,K=feat_channels)


# ================= Fine-tuning (unchanged from v2) ===================================
...
def finetune_extractor_on_points(factor_tif,points_path,epochs=FT_EPOCHS,lr=FT_LR,
    ...
    # Without this, model + AdamW moments (~4.5 GB) stay allocated when
    # write_deep_feature_raster() loads a second model instance → OOM.
...
# ================= Predict probability map (unchanged from v2) =======================
def predict_probability_map(deep_feats_tif=DEEP_FEATS_TIF, factors_tif=FACTOR_TIF,
    ...
    with rasterio.open(deep_feats_tif) as dsrc,rasterio.open(factors_tif) as fsrc:
        ...
        meta=dsrc.meta.copy(); meta.update(count=1,dtype='float32',tiled=True,
                                            blockxsize=TIFF_BLOCK,blockysize=TIFF_BLOCK,
                                            compress=TIFF_COMPRESS,bigtiff=TIFF_BIGTIFF)
            ...
            with rasterio.open(out_tif,'w',**meta) as dst,tqdm(total=total,desc="[C] Map") as pbar:
                for y in range(0,height,tile):
                    for x in range(0,width,tile):
                        ...
                        D=dsrc.read(window=win,out_dtype='float32')
                        F=fsrc.read(window=win,out_dtype='float32')
                        ...
                        if feature_names is not None and len(feature_names)==Xp.shape[1]:
                            X_pred=pd.DataFrame(Xp,columns=feature_names)
                        else: X_pred=Xp
                        dst.write(clf.predict_proba(X_pred)[:,1].astype('float32').reshape(H,W),1,window=win)
                        pbar.update(1)
...
if __name__ == "__main__":
    ...
    _clear_gpu("after feature extraction")   # free CNN before LightGBM training

    payload = train_lgbm_from_points(DEEP_FEATS_TIF, FACTOR_TIF, POINTS_PATH,
